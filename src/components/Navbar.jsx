...
const Navbar = () => {
  ...
  return (
    <div className='absolute top-0 left-0 w-full z-10'>
        ...
        {/* ----- mobile menu----*/}
        <div className={`md:hidden ${showMobileMenu ? 'fixed w-full h-screen' :'h-0 w-0'} right-0 left-0 top-0 overflow-hidden bg-white transition-all`}>
          <div className='flex justify-end p-6 cursor-pointer'>
            <img onClick={()=>setshowMobileMenu(false)} src={cross} className ='w-7' alt="" />
          </div>
          <ul className='flex flex-col items-center gap-2 mt-5 px-5 text-lg font-medium'>
            <a onClick={()=>setshowMobileMenu(false)} href="#Header" className='px-4 py2 rounded-full inline-block'>Home</a>
            <a onClick={()=>setshowMobileMenu(false)} href="#Abouts" className='px-4 py2 rounded-full inline-block'>About</a>
